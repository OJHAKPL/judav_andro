		...
		if(user_id){
			...
			$.ajax({
				...
				success: function(data){ 
					...
					if(!dataArr.error) {
						$('#heddingHtml').html(dataArr.success);
					}  
				},
				dataType: 'html'
			});
	...
	function favouriteOrganizer(id) { 
		...
		if(user_id){
				...
				//$( ".haver_cls_"+id ).removeClass( "after-bg" ).addClass( "after-bg-active" );
				//$( ".haver_cls_"+id ).removeClass( "after-bg-active" ).addClass( "after-bg" );
